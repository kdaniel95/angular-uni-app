...
export class TeacherSemesterCourseTable{
  ...
  public static teacherSemesterCourses: TeacherSemesterCourse[] = this._teacherSemesterCourses.map(tsc => {
    const teacher = TeacherTable.teachers.find(t => t.id === tsc.teacherId);
    const semester = SemesterTable.semesters.find(s => s.id === tsc.semesterId);
