...
teachersSemesterCourses$: Observable<TeacherSemesterCourseModel[]> = this.store.pipe(
  select(selectTeacherSemesterCourses)
);
...
ngOnInit() {
  this.route.paramMap.subscribe( params => {
      ...
      return this.store.dispatch(teacherSemesterCoursesRequestedAction({teacherId: +teacherId,  semesterId: +semesterId}))});
}
